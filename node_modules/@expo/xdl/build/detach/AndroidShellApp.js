...
function xmlWeirdAndroidEscape(original) {
  const noAmps = (0, _replaceString().default)(original, '&', '&amp;');
  const noLt = (0, _replaceString().default)(noAmps, '<', '&lt;');
  const noGt = (0, _replaceString().default)(noLt, '>', '&gt;');
  const noApos = (0, _replaceString().default)(noGt, '"', '\\"');
...
exports.updateAndroidShellAppAsync = async function updateAndroidShellAppAsync(args) {
  ...
  const shellPath = _path().default.join(exponentDirectory(workingDir), 'android-shell-app');

  await _fsExtra().default.remove(_path().default.join(shellPath, 'app', 'src', 'main', 'assets', ExponentTools().getManifestFileNameForSdkVersion(sdkVersion)));
  await _fsExtra().default.writeFileSync(_path().default.join(shellPath, 'app', 'src', 'main', 'assets', ExponentTools().getManifestFileNameForSdkVersion(sdkVersion)), JSON.stringify(manifest));
  await _fsExtra().default.remove(_path().default.join(shellPath, 'app', 'src', 'main', 'assets', ExponentTools().getBundleFileNameForSdkVersion(sdkVersion)));
  await saveUrlToPathAsync(bundleUrl, _path().default.join(shellPath, 'app', 'src', 'main', 'assets', ExponentTools().getBundleFileNameForSdkVersion(sdkVersion)));
  ...
  await regexFileAsync('// ADD EMBEDDED RESPONSES HERE', `
    ...
    embeddedResponses.add(new Constants.EmbeddedResponse("${fullManifestUrl}", "assets://${ExponentTools().getManifestFileNameForSdkVersion(sdkVersion)}", "application/json"));
    embeddedResponses.add(new Constants.EmbeddedResponse("${bundleUrl}", "assets://${ExponentTools().getBundleFileNameForSdkVersion(sdkVersion)}", "application/javascript"));
...
function backgroundImagesForApp(shellPath, manifest, isDetached, majorSdkVersion) {
  ...
  // ]
  const splashImageFilename = majorSdkVersion >= 39 ? 'splashscreen_image.png' : 'shell_launch_background_image.png';

  const basePath = _path().default.join(shellPath, 'app', 'src', 'main', 'res');
  ...
  if (splash) {
    const results = imageKeys.reduce(function (acc, imageKey) {
      ...
      if (url) {
        acc.push({
          url,
          path: _path().default.join(basePath, `drawable-${imageKey}`, splashImageFilename)
        });
      }
  ...
  const url = isDetached ? manifest.splash && manifest.splash.image : manifest.splash && manifest.splash.imageUrl;

  if (url) {
    return [{
      url,
      path: _path().default.join(basePath, 'drawable-xxxhdpi', splashImageFilename)
    }];
  }
...
function getSplashImageResizeMode(manifest) {
  return manifest.android && manifest.android.splash && manifest.android.splash.resizeMode || manifest.splash && manifest.splash.resizeMode;
}
...
exports.createAndroidShellAppAsync = async function createAndroidShellAppAsync(args) {
  ...
  const androidSrcPath = _path().default.join(exponentDir, 'android');

  const shellPath = _path().default.join(exponentDir, 'android-shell-app');
  ...
  if (privateConfigFile) {
    const privateConfigContents = await _fsExtra().default.readFile(privateConfigFile, 'utf8');
  ...
  const buildFlags = _StandaloneBuildFlags().default.createAndroid(configuration, androidBuildConfiguration);

  const context = _StandaloneContext().default.createServiceContext(androidSrcPath, null, manifest, privateConfig,
  /* testEnvironment */
...
async function runShellAppModificationsAsync(context, sdkVersion, buildMode) {
  var _manifest$detach, _manifest$android, _manifest$notificatio, _manifest$notificatio2;
  ...
  const multiPlatformSchemes = ensureStringArray(manifest.scheme || ((_manifest$detach = manifest.detach) === null || _manifest$detach === void 0 ? void 0 : _manifest$detach.scheme));
  const androidSchemes = ensureStringArray((_manifest$android = manifest.android) === null || _manifest$android === void 0 ? void 0 : _manifest$android.scheme);
  ...
  const backgroundImages = backgroundImagesForApp(shellPath, manifest, isRunningInUserContext, majorSdkVersion);
  ...
  const updatesCheckAutomaticallyDisabled = manifest.updates && manifest.checkAutomatically === 'ON_ERROR_RECOVERY';
  const fallbackToCacheTimeout = manifest.updates && manifest.updates.fallbackToCacheTimeout; // Clean build directories
  ...
  if (isDetached) {
    const rootBuildGradle = _path().default.join(shellPath, 'build.gradle');

    const appBuildGradle = _path().default.join(shellPath, 'app', 'build.gradle');
    ...
    await deleteLinesInFileAsync('WHEN_DISTRIBUTING_REMOVE_FROM_HERE', 'WHEN_DISTRIBUTING_REMOVE_TO_HERE', rootBuildGradle);

    if (majorSdkVersion >= 33) {
      const settingsGradle = _path().default.join(shellPath, 'settings.gradle');
  ...
  await regexFileAsync(`javaMaxHeapSize "8g"`, `javaMaxHeapSize "6g"`, _path().default.join(shellPath, 'app', 'build.gradle')); // TODO: probably don't need this in both places
  ...
  if (majorSdkVersion >= 39 && getSplashImageResizeMode(manifest) !== 'native') {
    ...
    await regexFileAsync(/<item>(.|\s)*?<\/item>/, '', _path().default.join(shellPath, 'app', 'src', 'main', 'res', 'drawable', 'splashscreen.xml'));
  } // Pass the correct SplashScreenImageResizeMode into the AppConstants


  if (majorSdkVersion >= 39) {
    await regexFileAsync(/SPLASH_SCREEN_IMAGE_RESIZE_MODE = SplashScreenImageResizeMode\..*?;/, `SPLASH_SCREEN_IMAGE_RESIZE_MODE = SplashScreenImageResizeMode.${(getSplashImageResizeMode(manifest) || 'contain').toUpperCase()};`, _path().default.join(shellPath, 'app/src/main/java/host/exp/exponent/generated/AppConstants.java'));
  ...
  if (majorSdkVersion >= 39 && updatesCheckAutomaticallyDisabled) {
    await regexFileAsync('UPDATES_CHECK_AUTOMATICALLY = true', 'UPDATES_CHECK_AUTOMATICALLY = false', _path().default.join(shellPath, 'app', 'src', 'main', 'java', 'host', 'exp', 'exponent', 'generated', 'AppConstants.java'));
  }

  if (majorSdkVersion >= 39 && fallbackToCacheTimeout) {
    await regexFileAsync('UPDATES_FALLBACK_TO_CACHE_TIMEOUT = 0', `UPDATES_FALLBACK_TO_CACHE_TIMEOUT = ${fallbackToCacheTimeout}`, _path().default.join(shellPath, 'app', 'src', 'main', 'java', 'host', 'exp', 'exponent', 'generated', 'AppConstants.java'));
  } // App name
  ...
  if (majorSdkVersion >= 39) {
    await regexFileAsync('"splashscreen_background">#FFFFFF', `"splashscreen_background">${splashBackgroundColor}`, _path().default.join(shellPath, 'app', 'src', 'main', 'res', 'values', 'colors.xml'));
  } else {
    await regexFileAsync('"splashBackground">#FFFFFF', `"splashBackground">${splashBackgroundColor}`, _path().default.join(shellPath, 'app', 'src', 'main', 'res', 'values', 'colors.xml'));
  ...
  if (manifest.android && manifest.android.allowBackup === false) {
    await regexFileAsync(`android:allowBackup="true"`, `android:allowBackup="false"`, _path().default.join(shellPath, 'app', 'src', 'main', 'AndroidManifest.xml'));
  } // Add permissions
  ...
  if (manifest.android && manifest.android.permissions) {
    ...
    const blacklist = [// module permissions - included by default / requires user to add to `android.permissions` list
    ...
    'android.permission.READ_PHONE_STATE', 'android.permission.SYSTEM_ALERT_WINDOW', // signature permissions - not available for 3rd party
  ...
  if (isFullManifest) {
    ...
    await regexFileAsync('// START EMBEDDED RESPONSES', `
      // START EMBEDDED RESPONSES
      embeddedResponses.add(new Constants.EmbeddedResponse("${fullManifestUrl}", "assets://${ExponentTools().getManifestFileNameForSdkVersion(sdkVersion)}", "application/json"));
      embeddedResponses.add(new Constants.EmbeddedResponse("${bundleUrl}", "assets://${ExponentTools().getBundleFileNameForSdkVersion(sdkVersion)}", "application/javascript"));`, _path().default.join(shellPath, 'app', 'src', 'main', 'java', 'host', 'exp', 'exponent', 'generated', 'AppConstants.java'));
  }
  ...
  (0, _AndroidIcons().createAndWriteIconsToPathAsync)(context, _path().default.join(shellPath, 'app', 'src', 'main', 'res'), isRunningInUserContext); // Set the tint color for icons in the notification tray
  // This is set to "#005eff" in the Expo client app, but
  // just to be safe we'll match any value with a regex

  await regexFileAsync(/"notification_icon_color">.*?</, `"notification_icon_color">${(_manifest$notificatio = (_manifest$notificatio2 = manifest.notification) === null || _manifest$notificatio2 === void 0 ? void 0 : _manifest$notificatio2.color) !== null && _manifest$notificatio !== void 0 ? _manifest$notificatio : '#ffffff'}<`, _path().default.join(shellPath, 'app', 'src', 'main', 'res', 'values', 'colors.xml')); // Splash Background

  if (backgroundImages && backgroundImages.length > 0) {
    // Delete the placeholder images
    const splashImageFilename = majorSdkVersion >= 39 ? 'splashscreen_image.png' : 'shell_launch_background_image.png';
    (0, _glob().sync)(`**/${splashImageFilename}'`, {
  ...
  if (privateConfig) {
    ...
    const googleMobileAdsAutoInit = privateConfig.googleMobileAdsAutoInit; // Branch
  ...
  if (parseSdkMajorVersion(sdkVersion) >= 37) {
    ...
    if (!manifest.android || !manifest.android.googleServicesFile) {
      ...
      await regexFileAsync('"package_name": "host.exp.exponent"', `"package_name": "${javaPackage}"`, _path().default.join(shellPath, 'app', 'google-services.json')); // Let's not modify values of the original google-services.json file
...
async function buildShellAppAsync(context, sdkVersion, buildType, buildMode, userProvidedGradleArgs) {
  const shellPath = shellPathForContext(context);
  ...
  if (isRelease) {
    ...
    await _fsExtra().default.copy(outputPath, context.build && context.build.android && context.build.android.outputFile || `/tmp/shell-debug.${ext}`);
    await ExponentTools().removeIfExists(outputPath);
  }
}
...
function addDetachedConfigToExp(exp, context) {
  ...
  const assetsDirectory = _path().default.join(shellPath, 'app', 'src', 'main', 'assets');
