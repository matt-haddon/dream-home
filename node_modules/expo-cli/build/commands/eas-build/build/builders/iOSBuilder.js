...
function _ios() {
  const data = require("../../utils/expoUpdates/ios");
...
class iOSBuilder {
  ...
  async ensureCredentialsAsync() {
    ...
    const bundleIdentifier = await (0, _ios2().getBundleIdentifier)(this.ctx.commandCtx.projectDir, this.ctx.commandCtx.exp);
  ...
  async ensureProjectConfiguredAsync() {
    ...
    await (0, _git2().modifyAndCommitAsync)(async () => {
      ...
      if ((0, _common().isExpoUpdatesInstalled)(projectDir)) {
        await (0, _ios().syncUpdatesConfigurationAsync)(projectDir, exp);
      }
    }, {
  ...
  async configureProjectAsync() {
    ...
    await (0, _git2().modifyAndCommitAsync)(async () => {
      ...
      if ((0, _common().isExpoUpdatesInstalled)(projectDir)) {
        await (0, _ios().configureUpdatesAsync)(projectDir, exp);
      }
    }, {
  ...
  async configureEasBuildAsync() {
    ...
    const bundleIdentifier = await (0, _ios2().getBundleIdentifier)(projectDir, exp);
  ...
  shouldLoadCredentials() {
    return this.ctx.buildProfile.workflow === _easBuildJob().Workflow.Managed && this.ctx.buildProfile.buildType !== 'simulator' || this.ctx.buildProfile.workflow === _easBuildJob().Workflow.Generic;
  }
